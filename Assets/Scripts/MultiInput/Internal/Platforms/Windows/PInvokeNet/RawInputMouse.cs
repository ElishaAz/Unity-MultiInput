...
namespace MultiInput.Internal.Platforms.Windows.PInvokeNet
{
    ...
    public struct RawMouse
    {
        ...
        {
            return
                $"{nameof(Flags)}: {Flags}, {nameof(ButtonFlags)}: {ButtonFlags}, {nameof(ButtonData)}: {ButtonData}, {nameof(RawButtons)}: {RawButtons}, {nameof(LastX)}: {LastX}, {nameof(LastY)}: {LastY}, {nameof(ExtraInformation)}: {ExtraInformation}";
        }
    }
}